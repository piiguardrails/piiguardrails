...
export class PiiGuardrails implements INodeType {
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		for (let i = 0; i < items.length; i++) {
			try {
				if (operation === 'mask') {
					let text = this.getNodeParameter('text', i) as any;
					...
					let unmaskText = this.getNodeParameter('unmaskText', i) as any;
					if (typeof unmaskText === 'object' && unmaskText !== null) {
						unmaskText = JSON.stringify(unmaskText);
					}
					let mapping = this.getNodeParameter('mapping', i) as any;
					...
					if (typeof mapping === 'string') {
						try {
							...
							throw new NodeOperationError(
								this.getNode(),
								'Mapping must be a valid JSON object or expression returning an object',
								{ itemIndex: i },
							);
						}
					}
					...
					let scanText = this.getNodeParameter('scanText', i) as any;
